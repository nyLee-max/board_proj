...
<meta charset="UTF-8">
<title>MVC 게시판</title>
<link rel="stylesheet" href="/board_proj/board/css/style1.css">
</head>
<body>
	<%-- ${articleList } --%>
	<!-- 게시판 리스트 -->
			...
			<c:forEach var="board" items="${articleList }">
				<tr>
					...
					<td><c:if test="${board.board_re_lev ne 0 }">
						...
						href="boardDetail.do?board_num=${board.board_num }&page=${pageInfo.page }">${board.board_subject }</a>
	...
	<section>
		<c:choose>
			<c:when test="${fn:length(articleList)!=0&&pageInfo.listCount>0 }">
				<section id="pageList">
					<c:choose>
						<c:when test="${pageInfo.page <= 1 }">[이전]&nbsp;
						</c:when>
						<c:otherwise>
